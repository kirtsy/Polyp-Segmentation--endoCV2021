...
from utils.utils import clip_gradient, adjust_lr, AvgMeter, draw_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch', type=int,
                        default=50, help='epoch number')  # 20
    parser.add_argument('--lr', type=float,
                        default=1e-4, help='learning rate')  # 1e-4
    parser.add_argument('--batchsize', type=int,
                        default=18, help='training batch size')  # 16
    parser.add_argument('--trainsize', type=int,
                        default=512, help='training dataset size')  # image siza  352   512  652
    parser.add_argument('--clip', type=float,
                        default=0.5, help='gradient clipping margin')
    parser.add_argument('--decay_rate', type=float,
                        default=0.1, help='decay rate of learning rate')  # 0.1
    parser.add_argument('--decay_epoch', type=int,
                        default=50, help='every n epochs decay learning rate')  # 50
    ...
    loss2_show, loss3_show, loss4_show, loss5_show, total_loss_show = [], [], [], [], []
    for epoch in range(1, opt.epoch, ):
        adjust_lr(optimizer, opt.lr, epoch, opt.decay_rate, opt.decay_epoch)
        save_label = 0
        if (epoch == (opt.epoch - 1)) | ((epoch + 1) % 50 == 0):
            save_label = 1
        loss2, loss3, loss4, loss5 = train(train_loader, model, optimizer, epoch, save_label)
        loss2_show.append(loss2)
        loss3_show.append(loss3)
        loss4_show.append(loss4)
        loss5_show.append(loss5)
        total_loss = loss2 + loss3 + loss4 + loss5
        total_loss_show.append(total_loss)

    print(loss2_show)
    print(total_loss_show)

    draw_loss(loss2_show, "Loss 2")
    draw_loss(loss3_show, "Loss 3")
    draw_loss(loss4_show, "Loss 4")
    draw_loss(loss5_show, "Loss 5")
    draw_loss(total_loss_show, "Total loss")